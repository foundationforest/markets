...
1. Pick a category and a name: a lowercase slug no file here uses. A new category is a new folder.
...
- the name isn't already here.

No other rule. `check.sh` checks both: it runs forest's validator on every file, and since a file lives at `<category>/<name>.json`, it checks each path against the file's name and category, then that no two files share a name.
